<template>
	<div class="home">
		...
		<div v-for="item in list" :key="item.state">
			{{ item.state | stateType(["已结束", "进行中", "已超时"]) }}
		</div>
		...
		<div>
			...
			<div v-if="uploadSrc">
				<p class="mt-10" style="word-wrap: break-word;">
					上传成功：{{ uploadSrc }}
				</p>

				<button class="mt-10" @click="copyText">复制地址</button>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		getList() {
			this.loadMore = false;
			let duration = [1000, 2000, 3000, 5000][
